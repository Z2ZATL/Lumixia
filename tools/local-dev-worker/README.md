...
| `golden/local-dev-worker-telemetry.fixture.json` | Committed deterministic telemetry fixture output for schema stability checks. |
| `localDevWorkerTelemetryGoldenCheck.ts` | Fixture-only telemetry golden checker; reads committed JSON, regenerates fixtures in memory, validates safety, and compares without upload, Docker, network, DB, or runtime writes. |
...
PR #32 adds a local-dev-only CLI wrapper under `tools/local-dev-worker`. It accepts only report-format flags and never accepts a command, image, container name, label, cleanup target, environment variable, workspace path, or secret as input.

```powershell
npm run dremo:worker:lifecycle:report
npm run dremo:worker:lifecycle:report:json
npm run dremo:worker:lifecycle:report:fixture
npm run dremo:worker:lifecycle:report:fixture:json
```
...
PR #34 adds operator-facing documentation for the current worker system, and PR #37 records the accepted architecture decision:

| Guide | Purpose |
| --- | --- |
| [ADR index](../../docs/adr/README.md) | Architecture Decision Records for worker and sandbox boundary decisions. |
| [ADR 0001: Dremo local-dev worker boundary and Docker smoke lifecycle](../../docs/adr/0001-dremo-local-dev-worker-boundary.md) | Accepted decision for keeping local-dev execution out of `src/`, behind exact reviewed capabilities, deterministic verification, and non-uploading telemetry fixtures. |
| [local-dev-worker-capability-registry.md](../../docs/dremo-code/local-dev-worker-capability-registry.md) | Operator-facing registry mapping executable, plan-only, report-only, schema-only, golden-check, and verification-only worker surfaces to files, fixtures, docs, and safety notes. |
...
PR #36 adds a committed deterministic telemetry fixture JSON file and a fixture-only checker:
