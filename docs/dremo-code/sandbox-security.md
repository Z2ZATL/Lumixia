...
Local-dev worker threat model note: [local-dev-worker-threat-model-v2.md](./local-dev-worker-threat-model-v2.md) documents the current trust boundaries, assets, threat categories, mitigations, residual risks, and future review gates for the Dremo local-dev worker and Docker smoke lifecycle. Future worker PR authors should use [local-dev-worker-threat-checklist.md](./local-dev-worker-threat-checklist.md) before changing capabilities, policies, reports, telemetry schema, or docs checks.
