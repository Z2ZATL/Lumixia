...
| Local Docker prototype scope | Partially resolved. `docker --version` and readiness-only `docker version --format "{{json .}}"` are the only Docker CLI probes allowed under reviewed local-dev configs; container execution, broader daemon-state probes, socket access, mounts, image commands, and runtime commands remain open and require separate security-reviewed PRs. |
...
| Execution | Still no arbitrary command execution and no Docker runtime/container execution. Keep `docker --version` and readiness-only `docker version --format "{{json .}}"` as the only Docker CLI probes. |
