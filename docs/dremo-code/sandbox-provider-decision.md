...
| Local Docker prototype scope | Partially resolved. `docker --version` is the only Docker CLI probe allowed under reviewed local-dev config; container execution, daemon-state probes, socket access, mounts, and runtime commands remain open and require separate security-reviewed PRs. |
...
| Execution | Still no arbitrary command execution and no Docker runtime/container execution. Keep `docker --version` as the only Docker CLI probe. |
