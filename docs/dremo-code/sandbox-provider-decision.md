...
| Goal | Validate the runner interface and event stream shape before choosing a hosted provider. |
...
Recommended PR #13: **Add sandbox runner interface without executing commands**.

The next PR should remain conservative:

| Scope | Recommendation |
| --- | --- |
| Interface | Add TypeScript/Deno interfaces for sandbox runner operations and policy decisions. |
| Config | Add non-secret config shape for allowed providers and local/dev flags. |
| Execution | No arbitrary command execution yet. No Docker/E2B/Daytona integration code yet. |
| Events | Define how runner methods would map to server-owned events. |
| Security | Keep service role out of runner payloads and document credential handoff boundaries. |
