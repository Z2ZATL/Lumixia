...
Container design-gate note: PR #25 adds local-dev container execution policy and plan models only. It defines future image, command, network, mount, resource, and security gates, plus a non-executed `dockerRunPreview`; it does not execute `docker run`, start containers, pull/build images, mount workspaces, or expose execution through `src/` or production UI.
...
| Local Docker prototype scope | Partially resolved. `docker --version` and readiness-only `docker version --format "{{json .}}"` are the only Docker CLI probes allowed under reviewed local-dev configs. PR #25 defines plan-only container gates, but container execution, socket access, mounts, image pull/build, and runtime commands remain open and require separate security-reviewed PRs. |
...
| Execution | Still no arbitrary command execution and no Docker runtime/container execution. Container execution is plan-only until a separate reviewed smoke execution PR. |
