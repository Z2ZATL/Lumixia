...
| Current status | PR #13 adds the TypeScript runner interface, static default policy, pure event mapping helpers, and a noop runner that blocks command requests. PR #14 adds pure validation helpers for command requests, paths, environment variables, resources, and output caps. PR #15 exposes browser-local policy smoke validation. PR #16 adds the Docker local-dev adapter skeleton/config without execution. PR #17 adds the Docker execution security checklist as a pre-execution gate. PR #18 adds explicit local-dev gates and command guards, but defers Docker invocation because `src/` is browser-bundled. PR #19 adds `tools/local-dev-worker/` as an out-of-bundle worker boundary with blocked/dry-run behavior only. PR #20 adds a dry-run adapter, request validation, deterministic trace metadata, fixtures, executable self-check harness, package verification scripts, and stronger boundary scanning while preserving `noExecution: true`. PR #21 adds a disabled-by-default capability manifest, manual review gate, readiness evaluator, and readiness fixtures. PR #22 adds the first manually gated local-dev non-Docker version command execution adapter while keeping Docker blocked and `src/` process-free. PR #23 adds a Docker-specific `docker --version` probe only. PR #24 adds Docker daemon readiness classification only through `docker version --format "{{json .}}"`. PR #25 adds plan-only Docker container design gates. PR #26 adds the first exact no-network/no-mount/non-root container smoke execution path for `alpine:3.20 echo hello`. PR #27 adds smoke result normalization, output sanitization, audit-safe records, and cleanup-risk metadata. PR #28 adds deterministic smoke container naming, static labels, cleanup plan preview, and cleanup review policy. PR #29 adds exact cleanup execution for `docker rm -f lumixia-dremo-smoke-echo`. PR #30 adds local-dev lifecycle orchestration across readiness, exact smoke, audit, and exact cleanup using existing reviewed adapters only. PR #31 adds sanitized Markdown and deterministic JSON report formatting for existing lifecycle results. PR #32 adds a local-dev-only CLI wrapper and deterministic dry-report fixture mode for those reports. PR #33 adds committed golden Markdown/JSON checks for fixture report stability. PR #34 adds [local-dev-worker-operator-guide.md](./local-dev-worker-operator-guide.md), [local-dev-worker-troubleshooting.md](./local-dev-worker-troubleshooting.md), [local-dev-worker-extension-playbook.md](./local-dev-worker-extension-playbook.md), and docs link verification; arbitrary Docker runtime, socket, mount, network, root user, image pull/build, workspace access, arbitrary cleanup, repo execution, browser/production paths, and broader daemon-state paths remain denied. |
| Exit criteria | Interface, adapter boundary, worker boundary, command guards, safety scan, and disabled-by-default execution gates exist before execution; first real local/dev worker prototype runs only allowlisted harmless commands with strict CPU, memory, timeout, output, artifact, network, and cleanup controls. |
| Risks | Secret leakage, weak isolation, runaway processes, excessive cost, or treating local Docker as production-ready. |
| Verification steps | Confirm browser-bundled sandbox code contains no Docker invocation or process execution APIs. Future worker execution PR must satisfy `docker-execution-security-checklist.md` and add proof for no production secrets in sandbox, command timeout tests, output truncation tests, blocked path tests, default-deny network tests, cleanup tests, rollback, and manual security review. Post-PR #34 work should stay reporting, telemetry, documentation, or operator-experience oriented before any broader container command surface. |
