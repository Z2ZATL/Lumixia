...
| Exit criteria | Interface and adapter boundary exist before execution; first real local/dev prototype runs only allowlisted harmless commands with strict CPU, memory, timeout, output, artifact, network, and cleanup controls. |
| Risks | Secret leakage, weak isolation, runaway processes, excessive cost, or treating local Docker as production-ready. |
| Verification steps | Confirm skeleton contains no Docker invocation or process execution APIs. Future execution PR must satisfy `docker-execution-security-checklist.md` and add proof for no production secrets in sandbox, command timeout tests, output truncation tests, blocked path tests, default-deny network tests, cleanup tests, rollback, and manual security review. |
